...
public class BlogServiceImpl implements BlogService {
    ...
    public String getLoginStatus(Authentication auth){
        if(auth != null){
            ...
            return email + "(" + roles.stream().map(grantedAuthority -> grantedAuthority.toString().replace("ROLE_", ""))
    ...
    public boolean isAdmin(Authentication auth){
        if(auth != null) {
            UserDetails userDetails = (UserDetails) auth.getPrincipal();
            return userDetails.getAuthorities().stream().anyMatch(o -> ((GrantedAuthority) o).getAuthority().equals("ROLE_ADMIN"));
        }
        return false;
    }
    ...
    public void updatePost(Post postToUpdate){
     ...
     postRepository.save(postFromDB); // gdy obiekt posta jest już w bazie danych to post jest aktualizowany\
    }
